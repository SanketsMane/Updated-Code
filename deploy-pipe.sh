...
echo "2. Uploading via SSH Pipe (Cat)..."
run_remote "mkdir -p $REMOTE_DIR"
# Pipe local file to remote file via SSH cat
# This avoids SCP/SFTP negotation issues
cat "$ARCHIVE_NAME" | ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no "$USER@$HOST" "cat > $REMOTE_DIR/$ARCHIVE_NAME"
